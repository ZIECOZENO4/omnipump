...
const GridCell = React.memo(() => (
  <div className="border border-[#373737]"></div>
));
...
const FirstAnimation: React.FC = () => {
  ...
  const animateMouse = useCallback(async () => {
    const animate = async () => {
      await mouseControls.start({
        x: Math.random() * 100 - 50,
        y: Math.random() * 100 - 50,
  ...
  const animateDrop = useCallback(async () => {
    const animate = async () => {
      await dropControls.start({
        scale: [0, 1],
        opacity: [1, 0],
  ...
  useEffect(() => {
    ...
    animateMouse();
    animateDrop();
  }, [animateBackground, animateMouse, animateDrop]);
