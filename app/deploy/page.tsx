...
export default function Component() {
  ...
  useEffect(() => {
    if (liquidity > 0) {
      ...
      setInitialPrice(price.toFixed(8));
    }
  }, [ethAmount, liquidity]);
        ...
        <div className="space-y-6">
          <div className='flex flex-col md:flex-row md:gap-6'>
            <div>
              ...
              <input type="text" placeholder="Token name" className="w-full bg-black rounded p-2 border border-gray-700" />
            </div>
            <div>
              <label className="block mb-2">Ticker</label>
              <input type="text" placeholder="Token ticker" className="w-full bg-black rounded p-2 border border-gray-700" />
            </div>
          </div>
       
          <div>
            <label className="block mb-2">Description</label>
            <textarea placeholder="Token description" className="w-full bg-black rounded p-2 border border-gray-700 h-24"></textarea>
          </div>
          <div>
          ...
          <div>
            ...
            <input 
              ...
              onChange={(e) => setLockPercentage(parseFloat(e.target.value) || 0)}
            />
          </div>
          <div className="bg-black rounded p-4 flex flex-col md:flex-row md:justify-between gap-4">
            ...
            <div className='md:w-1/2 w-full flex flex-col'>
              <p className="mt-4">
                ...
                The initial price will be approximately: {initialPrice} ETH per token
              </p>
            </div>
          </div>
          <button 
          ...
          {showMore && (
            <>
              <div>
                ...
                <input type="text" placeholder="https://twitter.com/..." className="w-full bg-black rounded p-2 border border-gray-700" />
              </div>
              <div>
                <label className="block mb-2">Telegram Link</label>
                <input type="text" placeholder="https://t.me/..." className="w-full bg-black rounded p-2 border border-gray-700" />
              </div>
              <div>
                <label className="block mb-2">Website</label>
                <input type="text" placeholder="https://..." className="w-full bg-black rounded p-2 border border-gray-700" />
              </div>
            </>
          )}
